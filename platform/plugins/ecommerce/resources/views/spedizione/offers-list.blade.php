...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12 mt-5">
                ...
                <div class="d-inline justify-content-end" style="float: right;margin-right: 10px;">
                    <a href="/admin/ecommerce/spedizione/create-offer" class="btn btn-primary">Nuova
                        Coupon</a>
                </div>
                    ...
                    <tbody>
                        @foreach ($offers as $offer)
                            ...
                            <tr data-id="{{ $offer->id }}" class="offerte-row">
                                <td>{{ $offer->id }}</td>
                                <td>{{ $offer->code }}</td>
                                <td class="offer-status">{!! $offer->status
                                ...
                                <td><span
                                        class="badge badge-info">{{ $offer->customers()->pluck('customer_id')->count() }}</span>
                                </td>
                                <td>
                                    <button class="delete-coupon btn btn-danger"
                                        onclick="deleteOffer({{ $offer->id }})"><i class="fa fa-trash"></i></button>
                                    <button class="btn btn-primary mr-2 export-coupon-customers"
                                        onclick="exportCustomers({{ $offer->id }})"><i class="fa fa-user"></i></button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <script>
        function deleteOffer(offerId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post('/admin/ecommerce/spedizione/delete', {
                            id: offerId
                        })
        ...
        function exportCustomers(id) {
            ...
            axios.post(`/admin/ecommerce/spedizione/customers_export`, {
                    id: id
                })
                .then((response) => {
                    ...
                    if (error.response && error.response.status === 404 && error.response.data.message ===
                        "Offer not found for the given user ID") {
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
