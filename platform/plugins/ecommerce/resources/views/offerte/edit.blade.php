...
@section('content')
    ...
    <div class="container" id="main-discount">
        <div class="max-width-1200 row"
            style="padding: 20px;background:white;border-radius: 10px;box-shadow: 1px 0px 6px 0px #888;">
                    ...
                    <tbody>
                        ...
                        <tr data-id="{{ $offer->id }}" class="offerte-row">
                            ...
                            @php
                                ...
                                if ($products->count() < 2) {
                                    $plannedOffersCount = $offerDetails
                                        ->where('product_id', $products[0]->id)
                                        ->where('status', 'planned')
                                        ->count();
                                }
                            @endphp
                            <td><input type="date" class="form-control editExpiringdate" placeholder="From Date"
                                    value={{ $offer->offer_expiring_date }}
                                @if (isset($plannedOffersCount)) @if ($plannedOffersCount < 1) @endif @else
                                    disabled @endif></td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="mt-3 col-md-12 poduct-section">
                ...
                @if ($products->count() < 2)
                    <div class="alert alert-info" role="alert">
                        La attivazioe/disattivazione su singolo prodotto non è possibile perché questo attiverà/disattiverà
                        l'intera offerta, per questo devi fare direttamente dalla sezione elenco dell'offerte
                    </div>
                @endif
                ...
                @foreach ($products as $product)
                    @php
                        ...
                        $gift_product = Product::find(
                            $offerDetails->where('product_id', $product->id)->first()->gift_product_id,
                        );
                        if ($gift_product) {
                    ...
                    <table data-id="{{ $product->id }}" class="product-table mb-3 table table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                @if ($offer->offer_type == 6)
                                    <th>Quantita</th>
                                @endif
                                @if ($offer->offer_type == 1 || $offer->offer_type == 3 || $offer->offer_type == 2 || $offer->offer_type == 6)
                                    <th>Prezzo scontato</th>
                                @endif
                                @if ($offer->offer_type == 5)
                                    <th>Prodotto collegato</th>
                                @endif
                        ...
                        <tbody>
                            <tr class="offerte-row">
                                ...
                                <td>
                                    <label class="switch">
                                        <input class='toggle-switch' type="checkbox"
                                            ...
                                            @if ($products->count() < 2) disabled @endif>
                                ...
                                @endif
                                @if ($offer->offer_type == 5)
                                    @if (isset($gift_product_name))
                                        <th>{{ $gift_product_name }}</th>
                                    @endif
                                @endif
                            </tr>
                        </tbody>
                        ...
                        <tbody>
                            @foreach ($customers as $customer)
                                ...
                                <tr data-id="{{ $customer->id }}" class="offerte-row">
                                    ...
                                    @endphp
                                    <td class="customer_agent">
                                        @if ($agent)
                                            {{ $agent->nome }} &nbsp {{ $agent->cognome }}
                                        @endif
                                    </td>
                                    <td><span
                                            class="badge badge-danger">{{ Regione::find($customer->region_id)->name }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
...
@push('header')
    <style>
        ...
        .regione-row input:checked+.slider:before,
        .agente-row input:checked+.slider:before,
        .strumenti-row input:checked+.slider:before {
        ...
        .regione-row .switch,
        .agente-row .switch,
        .strumenti-row .switch {
            width: 23px;
            height: 14px;
        }

        .regione-row .switch .slider:before,
        .agente-row .switch .slider:before,
        .strumenti-row .switch .slider:before {
    ...
    <script>
        ...
        $(document).on('change', '.product-table .toggle-switch', function(evt) {
            ...
            if (totalCheckedProductTables == 0) {
                ...
                axios
                    .post("https://marigopharma.it/admin/ecommerce/offerte/deactiveProductInoffer", {
            ...
            if (totalCheckedCustomerTables == 0) {
                ...
                if (!prevToggleSwitchChecked) {
                    ...
                    if (totalCheckedToggleSwitches != 0) {
                        axios
                            .post("https://marigopharma.it/admin/ecommerce/offerte/deactiveCustomerInoffer", {
                                product_id: productTableId,
        ...
        $(document).on('change', '.editExpiringdate', function(evt) {
            ...
            axios.post('https://marigopharma.it/admin/ecommerce/offerte/updateExpirationDate', {
                    offer_id: offerId,
